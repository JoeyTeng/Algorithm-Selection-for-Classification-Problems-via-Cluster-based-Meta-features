...
class Models(object):
    ...
    def knn(X, y):
        ...
        model = KNeighborsClassifier(
            n_neighbors=len(X), weights='distance', n_jobs=-1)
...
class MetaFeature(object):
    ...
    def N(self):
        ...
        Number of examples
        """
        return self.__universal_setter__('N', lambda: len(self.y))
    ...
    def q(self):
        ...
        Number of classes
        """
        return self.__universal_setter__('q', lambda: len(np.unique(self.y)))
    ...
    def corr_abs(self):
        ...
        Only applicable for continuous classes.
        Mean abs(R value) of the attributes
        Relation between the dependent var and the set of independent vars
            normalized error.
        """
        pass
    ...
    def kurtosis(self):
        ...
        """
        return self.__universal_setter__('kurtosis', lambda: stats.kurtosis(self.X, fisher=True, bias=False))  # fisher = true (normal = 0), bias correction enabled. CHECK!
...
def paired_t_test(scores):
    ...
    result = list(map((lambda x: [stats.ttest_rel(x, b)
                                  for b in scores.values()]), scores.values()))
